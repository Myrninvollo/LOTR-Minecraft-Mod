...
public class LOTRWorldGenMangrove extends WorldGenAbstractTree
{
    ...
    {
		super(flag);
		woodID = LOTRMod.wood3;
		woodMeta = 3;
		leafID = LOTRMod.leaves3;
		leafMeta = 3;
	}
    ...
    public boolean generate(World world, Random random, int i, int j, int k)
    {
        ...
        if (j >= 1 && j + height + 1 <= 256)
        {
                ...
                for (int i1 = i - range; i1 <= i + range && flag; i1++)
                {
                    for (int k1 = k - range; k1 <= k + range && flag; k1++)
                    {
                        if (j1 >= 0 && j1 < 256)
                        {
                            ...
                            if (block.getMaterial() != Material.air && block.getMaterial() != Material.water && !block.isLeaves(world, i1, j1, k1) && block != Blocks.grass && block != Blocks.dirt && !block.isWood(world, i1, j1, k1))
                            {
                                flag = false;
                            }
                        }
            ...
            else
            {
				...
				boolean canGrow = (below == Blocks.grass || below == Blocks.dirt || below == Blocks.sand) && j < 256 - height - 1;
    ...
    private void growVines(World world, Random random, int i, int j, int k, int meta)
    {
        ...
        {
            j--;
            if (!world.getBlock(i, j, k).isAir(world, i, j, k) || vines >= 2 + random.nextInt(3))
            {
                return;
            }
            setBlockAndNotifyAdequately(world, i, j, k, Blocks.vine, meta);
