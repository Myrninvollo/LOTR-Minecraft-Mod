...
public class LOTRWorldGenUrukCamp extends LOTRWorldGenStructureBase
{
	private LOTRWorldGenTentBase urukTentGen = new LOTRWorldGenUrukTent(false);
	private LOTRWorldGenTentBase urukForgeTentGen = new LOTRWorldGenUrukForgeTent(false);
	
	public LOTRWorldGenUrukCamp()
	{
		super(false);
	}
	...
	{
		if (world.getBiomeGenForCoords(i, k) != LOTRBiome.rohanUrukHighlands || world.getBlock(i, j - 1, k) != Blocks.grass)
		{
			return false;
		}
		...
		setBlockAndNotifyAdequately(world, i, highestHeight, k, LOTRMod.urukTable, 0);
		
		LOTREntityUrukHai uruk = random.nextBoolean() ? new LOTREntityUrukHaiTrader(world) : new LOTREntityUrukHaiMercenaryCaptain(world);
		uruk.setLocationAndAngles(i + 0.5D, highestHeight + 1D, k + 0.5D, world.rand.nextFloat() * 360F, 0F);
		uruk.onSpawnWithEgg(null);
		uruk.setHomeArea(i, highestHeight, k, 24);
		world.spawnEntityInWorld(uruk);
		...
		for (int l = 0; l < 4; l++)
		{
			...
			{
				urukForgeTentGen.generateWithSetRotation(world, random, i1, j1, k1, l);
			}
			else
			{
				urukTentGen.generateWithSetRotation(world, random, i1, j1, k1, l);
			}
		}
		...
		if (usingPlayer == null)
		{
			LOTRLevelData.urukCampLocations.add(new ChunkCoordinates(i, j, k));
		}
